...
def process_labels(labels, word_to_idx, num_classes=379):#여러 텍스트 라벨을 받아 멀티-핫 인코딩으로 변환하는 함수
    ...
    for label in labels:
        ...
        if label in word_to_idx:
            ...
            print("one_hot_labels is",one_hot_labels) #debug
        else:
...
#없앨지말지,,,, 왜있음 이거
def process_labels1(batch_labels, word_to_idx, num_classes): #트레이닝과 데이터셋 생성시 이용한 라벨링 방식과 텍스트 인풋이 달라서 부득이하게 추가 ??????
    ...
    #         else:
    #             print(f"Warning: Label {label} not found in word_to_idx1.")
...
# Dataset 불러오기
dataset = Sprites(conf.data_dir, 'test', n=100000, canvas_size=128, transform=transform)
...
test_num=int(input("몇 개의 이미지를 평가하시겠습니까?: "))
...
for idx, (image, label) in enumerate(data_loader):
    if idx < test_num:  # 첫 번째 이미지를 사용
        ...
        # plt.axis('off')
        # plt.show()
        original_image.save("test_image.png")  # PIL 이미지 저장
        ...
        new_labels = [new_label.strip() for label in new_label.strip('[]').split(',')] #자연어 멀티핫 라벨 분할
        
        one_hot_labels = process_labels(new_labels, word_to_idx, num_classes)
        print(one_hot_labels)

        # 원본이미지를 원핫인코딩
        # one_hot_labels1 = process_labels1(label, word_to_idx1, num_classes1)
        # print(one_hot_labels1)
        label = label.squeeze(0).to(torch.long)
        ...
        # 원하는 출력 형태로 변경: (1, num_classes)
        one_hot_labels1 = one_hot.unsqueeze(0)

        print(one_hot_labels1.shape)  # torch.Size([1, 300])
        print(one_hot_labels1)
        ...
        # plt.axis('off')
        # plt.show()
        final_image.save("test_image2.png")  # PIL 이미지 저장
